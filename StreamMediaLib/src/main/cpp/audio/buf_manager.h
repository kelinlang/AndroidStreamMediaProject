...
class ProducerConsumerQueue {
 public:
  ...
  bool push(const F& writer) {
    bool result = false;
    int readptr = read_.load(std::memory_order_acquire);//本线程中，后续的读操作必须在本条原子操作完成后进行
    int writeptr = write_.load(std::memory_order_relaxed);//不对执行顺序做任何保证
    ...
    if (space >= 1) {
      ...
      // writer
      if (writer(buffer_.get() + (writeptr % size_))) {//writer 返回true才会
        ++writeptr;
...
__inline__ sample_buf* allocateSampleBufs(uint32_t count, uint32_t sizeInByte) {
  ...
  sample_buf* bufs = new sample_buf[count];//一维数组
  assert(bufs);
