package com.lib.commonlib.utils.bytes;
...
public class ByteTools {
	...
	public static void appendByteToBig(int s, byte[] dat, int pos)
	{
		int index = pos;
		for (int i = 0; i < 1; i++)
		{
			dat[index++] = (byte) (s >> ((0-i) * 8) & 0xFF);
		}
	}
	...
	public static String parseStringGB2312FromFixedLengthArray(byte[] data, int pos, int length)
	{
		byte[] bytes = new byte[length];
		System.arraycopy(data, pos, bytes, 0, length);
		int bytesLength = bytes.length;
		int strLength = 0;
		while (strLength < bytesLength){
			if (bytes[strLength] == 00){
				break;
			}
			strLength ++;
		}
		...
		System.arraycopy(bytes, 0, strBytes, 0, strBytes.length);

		String returnString = null;
		try {
			returnString =  new String(strBytes, "GB2312");
		} catch (UnsupportedEncodingException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	 ...
	 * @return 把 i 的第pos 位变为0后的值
	 */
	public static long longDo0(long i,int pos){
		pos ++;
		long src= Long.MAX_VALUE^(1<<(pos-1));
		return i&src;
	}
	/**
	 *
	 ...
	 * @return 把 i 的第pos 位变为1后的值
	 */
	public static long longDo1(long i,int pos){
		pos ++;
		return i|(1<<pos-1);
	}
     ...
     * @return
     */
	public static int getIntPosValue(int i,int pos){
		i = i >>pos;
		i = i | 1;
		return i;
	}
	 ...
	 * @return
	 */
	public static long getLongPosValue(long i,int pos){
		i = i >>pos;
		i = i | 1;
		return i;
	}
     ...
     * @return
     */
	public static int getIntPosValue(int i,int startPos, int endPos){
		i = i >> startPos;
		int num = endPos - startPos;
		int mask = 0xFFFF>>(31 - num);
		i = i | mask;
		return i;
	}
	...
	public static byte[] ip2BinArray(String ip){
		byte[] ipArray = new byte[4];
		String[] arr = ip.split("\\.");
		int pos = 0;
		for (String str : arr) {
			ipArray[pos++] = intToHexByte(Integer.valueOf(str));
		}
		return ipArray;
	}

	public static byte[] port2BinArray(int port){
		byte[] portArray = new byte[2];
		appendShortToBig(port,portArray,0);
		return portArray;
	}
}
